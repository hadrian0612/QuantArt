...
# --------------------------- 日志配置 --------------------------- #
LOG_FILE = Path("./logs/fetch_baostock.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("incremental_baostock_update")

# Baostock 接口非线程安全，请求时加锁
...
def _code_to_bs(code: str) -> str:
    """6 位 A 股代码转为 Baostock 代码：6 开头 sh.xxxxxx，否则 sz.xxxxxx"""
...
def _get_kline_baostock(code: str, start: str, end: str) -> pd.DataFrame:
    """
    使用 Baostock 获取 A 股日线数据（前复权），返回与现有 CSV 兼容的格式：
    ...
    with _bs_lock:
        ...
        if rs.error_code != "0":
            raise RuntimeError(f"Baostock 返回错误: {rs.error_msg} (code={rs.error_code})")
    ...
    # 列名已为 date, open, high, low, close, volume；输出顺序与现有 CSV 一致
    required_cols = ["date", "open", "close", "high", "low", "volume"]
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        logger.warning("%s (A) Baostock 返回数据缺少列: %s，可用列: %s", code, missing, df.columns.tolist())
        return pd.DataFrame()
...
def update_one_incremental(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            new_df = _get_kline_baostock(code, start_date, end_date)
            ...
            if new_df.empty and end_date_is_today and last_date is not None:
                ...
                if pd.to_datetime(start_date) <= pd.to_datetime(yesterday):
                    ...
                    if request_delay > 0:
                        time.sleep(random.uniform(request_delay * 0.5, request_delay * 1.5))
                    new_df = _get_kline_baostock(code, start_date, yesterday)
...
def main():
    parser = argparse.ArgumentParser(
        description="使用 Baostock 对已存在的 A 股 K 线 CSV 做增量更新（从最后日期到指定结束日期）"
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=["A", "HK", "US", "all"],
        default="A",
        help="市场类型（当前脚本仅支持 A 股，其他市场会忽略，仅为参数兼容）",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=4,
        help="并发线程数（默认：4；Baostock 请求已串行化，多线程主要用于 IO）",
    )
    parser.add_argument(
        "--request-delay",
        type=float,
        default=0.3,
        metavar="SEC",
        help="每次请求前的随机延时基数（秒），实际延时为该值的 0.5~1.5 倍（默认：0.3）",
    )
    ...
    if args.market in {"HK", "US", "all"}:
        logger.warning(
            "Baostock 增量脚本当前仅支持 A 股（A-shares 目录），其他市场将被忽略。"
        )
    ...
    lg = bs.login()
    if lg.error_code != "0":
        logger.error("Baostock 登录失败: %s (code=%s)", lg.error_msg, lg.error_code)
        sys.exit(1)

    try:
        logger.info("=" * 80)
        logger.info("开始增量更新 A 股 K 线 | 数据源: Baostock | 结束日期: %s", end_date)
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            pbar = tqdm(total=len(tasks), desc="Baostock 更新进度")
            try:
        ...
        if failed_tasks:
            ...
            with ThreadPoolExecutor(max_workers=min(2, args.workers)) as executor:
                ...
                pbar = tqdm(total=len(failed_tasks), desc="Baostock 重试进度")
                try:
                    for future in as_completed(future_to_task):
                        code, csv_path = future_to_task[future]
                        try:
...
# 使用 Baostock 将 A 股日线更新到今天
python incremental_baostock_update.py --data-dir ./data --end today --default-start 20250101 --workers 4 --request-delay 0.3

# 仅更新 A 股
python incremental_baostock_update.py --data-dir ./data --market A --end today
"""
